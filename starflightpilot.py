...
HEADER_EMBED = {
    ...
    "description": "ðŸŽ›ï¸ **Announcement**",
...
FOOTER_EMBED = {
    ...
    "description": "ðŸš€ **pls invite ppl to join our discord server and help us grow!**\n\n[Click here](https://discord.gg/4QzQYeuApB) to join!",
...
async def on_ready():
    ...
    logger.info(f"ðŸš€ Online as {bot.user}")
...
def staff_only():
    ...
    async def predicate(interaction: discord.Interaction):
        ...
        await interaction.response.send_message("âŒ Staff only.", ephemeral=True)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if EmbedManager.save(self.name, data):
            ...
            await interaction.response.send_message(
                ...
                f"âœ… Embed **{self.name}** saved to database.",
...
async def embed_post(interaction: discord.Interaction, name: str, channel: Optional[discord.TextChannel] = None):
    ...
    if not d:
        ...
        return await interaction.response.send_message("âŒ Embed not found.", ephemeral=True)
...
async def embed_list(interaction: discord.Interaction):
    ...
    if not embeds:
        ...
        return await interaction.response.send_message("ðŸ“­ No embeds saved.", ephemeral=True)

    

    embed = discord.Embed(

        title="ðŸ“‹ Saved Embeds",
...
async def embed_delete(interaction: discord.Interaction, name: str):
    ...
    if EmbedManager.delete(name):
        ...
        await interaction.response.send_message("âŒ Embed not found.", ephemeral=True)
...
async def backup_ship(interaction: discord.Interaction):
    ...
    if BackupManager.save_to_db(interaction.guild.id, data):
        ...
        await interaction.followup.send("ðŸ’¾ Ship backed up to Railway database.", ephemeral=False)

    else:

        await interaction.followup.send("âŒ Failed to save backup to database.", ephemeral=False)
...
async def restore_ship(interaction: discord.Interaction):
    ...
    if not data:
        ...
        return await interaction.followup.send("âš ï¸ No ship backup found in database.", ephemeral=True)
    ...
    try:
        ...
        await interaction.followup.send("ðŸ› ï¸ Restoration complete! Roles and channels have been recreated.", ephemeral=True)

    except Exception as e:

        logger.error(f"Restore Error: {e}")

        await interaction.followup.send(f"âŒ An error occurred during restoration: {e}", ephemeral=True)
...
async def sync_tree(interaction: discord.Interaction):
    ...
    await interaction.followup.send("ðŸ“¡ Slash commands synced.", ephemeral=True)
...
class PlushieModal(discord.ui.Modal, title="Register Plushie"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message("ðŸ§¸ Plushie registered!", ephemeral=True)
...
class PlushieEditModal(discord.ui.Modal, title="Edit Plushie"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if PlushieManager.update(self.user_id, self.name, updates, image):
            ...
            await interaction.response.send_message("âŒ Failed to update plushie.", ephemeral=True)
...
async def plushie_edit(interaction: discord.Interaction, name: str, photo: Optional[discord.Attachment] = None):
    ...
    if not p:
        ...
        return await interaction.response.send_message("âŒ You don't have a plushie with that name.", ephemeral=True)
...
async def plushie_info(interaction: discord.Interaction, owner: discord.Member, name: str):
    ...
    if not p:
        ...
        return await interaction.response.send_message("âŒ Not found.", ephemeral=True)
...
async def plushie_list(interaction: discord.Interaction, owner: Optional[discord.Member] = None):
    ...
    embed = discord.Embed(
        ...
        title=f"ðŸ§¸ {target.display_name}'s Plushies",
...
class MissionManager:
    ...
    def load_missions() -> List[str]:
        ...
        try:
            ...
            return ["🚀 Take a break and stretch for 30 seconds!"]
...
async def plushie_remove(interaction: discord.Interaction, name: str):
    ...
    if PlushieManager.remove(interaction.user.id, name):
        ...
        await interaction.response.send_message("âŒ Plushie not found.", ephemeral=True)
...
async def stardate(interaction: discord.Interaction):
    ...
    # Calculate stardate (Star Trek style)
    now = datetime.now(timezone.utc)
    # Formula: (current year - 2323) * 1000 + day of year
